...
def parse_proto_example(proto):
    ...
    example['image'] = tf.image.resize_images(example['image'], tf.constant([600, 600]))
...
def augmented_train(image, label):
    ...
    image = tf.contrib.image.rotate(image , 60 * math.pi / 180 , interpolation='BILINEAR', name = None) # rotate
    image = tf.image.random_brightness(image, 0.5, seed=None)                    # brightness and contrast
    image = tf.image.random_contrast(image, 0.2, 1.2, seed=None)
    image = tf.image.random_crop(image,[500,500,3], seed=None,name=None)         # crop
...
class Validation(tf.keras.callbacks.Callback):
    def __init__(self, log_dir, validation_files, batch_size):
        ...
        validation_dataset = create_augmented_dataset(validation_files, batch_size)
